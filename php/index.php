...
<div class="container" style="width:60%;">
	<h2 align="center">Carro de la compra</h2>
    <?php
	...
	if(mysqli_num_rows($result) > 0)
	{
		while($row = mysqli_fetch_array($result))
		{
            ...
            <h5 class="text-danger"> <?php echo $row["salary"]; ?>€</h5>
	...
	if(!empty($_SESSION["cart"]))
	{
		...
		foreach($_SESSION["cart"] as $keys => $values) // LOGICA DEL CARRO DE LA COMPRA
		{
            ...
            <td><?php echo number_format($values["item_quantity"] * $values["product_salary"], 2); ?>€</td>
            <td><a href="carro.php?action=delete&id=<?php echo $values["product_id"]; ?>"><span class="text-danger">X</span></a></td>
            </tr>
            <?php 
			$total = $total + ($values["item_quantity"] * $values["product_salary"]);
		}
		?>
        <tr>
